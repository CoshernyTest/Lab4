...
int[,] field = initial_field;
//Console.WriteLine("START FIELD:\n" + GetMatrixInText(field));
foreach (string dir in move_dirs)
{
    field = MoveFieldInDir(field, dir);
...
{
    string matrix_in_text = GetMatrixInText(field);
...
//
int[,] MoveFieldInDir(int[,] field, string dir)
{
    int[,] new_field = field;
...
//
int[] ShiftLineInDirection(int[] line, string dir)
{
...
string GetMatrixInText(int[,] field)
